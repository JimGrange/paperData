...
getRTs <- function(data, minRT, sd){
  ...
  colnames(rtData) <- c("subject", 
                        "abaRepArrows", "abaSwArrows", 
                        "cbaRepArrows", "cbaSwArrows", 
                        "abaRepShapes", "abaSwShapes", 
                        "cbaRepShapes", "cbaSwShapes")
  ...
  for(sub in subjects){
    ...
    # do arrow trials
    subData <- subset(subData, subData$condition == "arrows")
    ...
    # do shape trials
    subData <- subset(data, data$subject == sub)
    subData <- subset(subData, subData$condition == "shapes")
...
doLong <- function(data){
  ...
  for(i in 1:nrow(data)){
    ...
    subjectData[1, ] <- c(data$subject[i], "arrows", "aba", "repetition", 
                          data$abaRepArrows[i])
    subjectData[2, ] <- c(data$subject[i], "arrows", "aba", "switch", 
                          data$abaSwArrows[i])
    subjectData[3, ] <- c(data$subject[i], "arrows", "cba", "repetition", 
                          data$cbaRepArrows[i])
    subjectData[4, ] <- c(data$subject[i], "arrows", "cba", "switch", 
                          data$cbaSwArrows[i])
    subjectData[5, ] <- c(data$subject[i], "shapes", "aba", "repetition", 
                          data$abaRepShapes[i])
    subjectData[6, ] <- c(data$subject[i], "shapes", "aba", "switch", 
                          data$abaSwShapes[i])
    subjectData[7, ] <- c(data$subject[i], "shapes", "cba", "repetition", 
                          data$cbaRepShapes[i])
    subjectData[8, ] <- c(data$subject[i], "shapes", "cba", "switch", 
                          data$cbaSwShapes[i])
